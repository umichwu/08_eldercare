...
-- ============================================================================
-- 版本: 5.1 (2025-01-21 更新)
-- 更新內容:
--   v5.1 新增:
--   - 社交功能（chat_messages, social_posts, post_likes, post_comments, social_notifications, pending_invitations）
--   - 群組聊天（chat_groups, chat_group_members, chat_group_invites）
--   - 每日提醒（reminder_categories, daily_reminders, daily_reminder_logs）
--   - 警示系統（alerts, family_settings）
--   - 欄位新增（user_profiles.device_info, medication_reminders.metadata, pending_invitations.last_sent_at）
--   v5.0:
--   - 短期用藥功能（藥物次數控制、序號標記）
--   - 圖片上傳功能（藥物拍照、心情日記配圖）
...
--   7. 社交互動系統（好友、聊天、動態貼文、通知）
--   8. 每日生活提醒（喝水、飲食、運動、睡眠、回診）
--   9. 家屬監控警示（用藥、健康、活動異常警示）
-- 角色: 長輩和家屬都可以與 AI 聊天
-- ============================================================================
...
-- ============================================================================
-- STEP 7.5: 社交功能表 (Social Features) - v5.1 新增
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 7.5.1 好友邀請表 (pending_invitations)
-- ----------------------------------------------------------------------------
CREATE TABLE public.pending_invitations (
    ...
    invitation_type TEXT DEFAULT 'friend' CHECK (invitation_type IN ('friend', 'family')),
    message TEXT,
...
CREATE INDEX idx_pending_invitations_invitee_email ON public.pending_invitations(invitee_email, status);
...
CREATE POLICY "Users can view own sent invitations"
    ...
    USING (inviter_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));

CREATE POLICY "Users can view invitations sent to their email"
    ON public.pending_invitations FOR SELECT
    USING (invitee_email IN (SELECT email FROM public.user_profiles WHERE auth_user_id = auth.uid()));

CREATE POLICY "Users can send invitations"
    ON public.pending_invitations FOR INSERT
    WITH CHECK (inviter_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));

CREATE POLICY "Users can update invitations"
    ON public.pending_invitations FOR UPDATE
    USING (
        inviter_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()) OR
        invitee_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid())
    );

COMMENT ON TABLE public.pending_invitations IS '好友邀請表 - 儲存好友和家屬邀請記錄';
COMMENT ON COLUMN public.pending_invitations.last_sent_at IS '最後發送時間（v5.1 新增）';

-- ----------------------------------------------------------------------------
-- 7.5.2 聊天訊息表 (chat_messages)
-- ----------------------------------------------------------------------------
...
CREATE POLICY "Users can view their own messages"
    ...
    USING (
        sender_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()) OR
        receiver_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid())
    );

CREATE POLICY "Users can send messages"
    ON public.chat_messages FOR INSERT
    WITH CHECK (sender_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));
...
CREATE POLICY "Users can delete their sent messages"
    ...
    USING (sender_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));

COMMENT ON TABLE public.chat_messages IS '聊天訊息表 - 支援一對一和群組訊息';
COMMENT ON COLUMN public.chat_messages.group_id IS '群組 ID（群組訊息時使用）';

-- ----------------------------------------------------------------------------
-- 7.5.3 聊天群組表 (chat_groups)
-- ----------------------------------------------------------------------------
...
CREATE POLICY "Users can create groups"
    ...
    WITH CHECK (created_by IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));
...
CREATE POLICY "Creators can delete groups"
    ...
    USING (created_by IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));

COMMENT ON TABLE public.chat_groups IS '聊天群組表';

-- ----------------------------------------------------------------------------
-- 7.5.4 群組成員表 (chat_group_members)
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.chat_group_members IS '群組成員表';

-- 添加外鍵約束（在 chat_groups 創建後）
...
-- 添加 chat_messages 的 group_id 外鍵
...
-- ----------------------------------------------------------------------------
-- 7.5.5 群組邀請表 (chat_group_invites)
-- ----------------------------------------------------------------------------
...
CREATE POLICY "Users can view own invites"
    ...
    USING (invitee_id IN (SELECT id FROM public.user_profiles WHERE auth_user_id = auth.uid()));
...
COMMENT ON TABLE public.chat_group_invites IS '群組邀請表';

-- ----------------------------------------------------------------------------
-- 7.5.6 社交貼文表 (social_posts)
-- ----------------------------------------------------------------------------
...
CREATE INDEX idx_social_posts_user ON public.social_posts(user_profile_id);
...
CREATE INDEX idx_social_posts_deleted_at ON public.social_posts(deleted_at) WHERE deleted_at IS NULL;
...
COMMENT ON TABLE public.social_posts IS '社交動態貼文表';

-- ----------------------------------------------------------------------------
-- 7.5.7 貼文按讚表 (post_likes)
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.post_likes IS '貼文按讚表';

-- ----------------------------------------------------------------------------
-- 7.5.8 貼文留言表 (post_comments)
-- ----------------------------------------------------------------------------
...
CREATE INDEX idx_post_comments_deleted_at ON public.post_comments(deleted_at) WHERE deleted_at IS NULL;
...
COMMENT ON TABLE public.post_comments IS '貼文留言表';

-- ----------------------------------------------------------------------------
-- 7.5.9 社交通知表 (social_notifications)
-- ----------------------------------------------------------------------------
...
CREATE INDEX idx_notifications_user ON public.social_notifications(user_id);
CREATE INDEX idx_notifications_is_read ON public.social_notifications(user_id, is_read) WHERE is_read = FALSE;
CREATE INDEX idx_notifications_created_at ON public.social_notifications(created_at DESC);
...
-- ============================================================================
-- STEP 7.6: 每日生活提醒系統 (Daily Routine Reminders) - v5.1 新增
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 7.6.1 提醒類別參考表 (reminder_categories)
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.reminder_categories IS '生活提醒類別參考表';

-- ----------------------------------------------------------------------------
-- 7.6.2 每日提醒主表 (daily_reminders)
-- ----------------------------------------------------------------------------
CREATE TABLE public.daily_reminders (
    ...
    missed_threshold_minutes INTEGER DEFAULT 30,
    total_reminders_sent INTEGER DEFAULT 0,
...
CREATE POLICY daily_reminders_elder_access ON public.daily_reminders
    FOR ALL
    USING (elder_id IN (SELECT id FROM public.elders WHERE auth_user_id = auth.uid()));
...
COMMENT ON COLUMN public.daily_reminders.category_specific_data IS '類別特定資料 (JSONB)';

-- ----------------------------------------------------------------------------
-- 7.6.3 每日提醒記錄表 (daily_reminder_logs)
-- ----------------------------------------------------------------------------
CREATE TABLE public.daily_reminder_logs (
    ...
    push_sent BOOLEAN DEFAULT false,
    push_sent_at TIMESTAMPTZ,
    push_delivery_status VARCHAR(50),
    family_notified BOOLEAN DEFAULT false,
    family_notified_at TIMESTAMPTZ,
    notes TEXT,
...
CREATE INDEX idx_daily_reminder_logs_pending ON public.daily_reminder_logs(status, scheduled_time) WHERE status = 'pending';
...
COMMENT ON TABLE public.daily_reminder_logs IS '生活提醒記錄表';
COMMENT ON COLUMN public.daily_reminder_logs.response_data IS '回饋資訊 (JSONB)';

-- ============================================================================
-- STEP 7.7: 監控警示系統 (Monitoring & Alerts) - v5.1 新增
-- ============================================================================
...
CREATE TABLE public.alerts (
    ...
    alert_type TEXT NOT NULL CHECK (alert_type IN ('medication', 'health', 'activity', 'emergency')),
    severity TEXT NOT NULL DEFAULT 'medium' CHECK (severity IN ('low', 'medium', 'high', 'critical')),
    title TEXT NOT NULL,
    description TEXT,
    related_medication_id UUID REFERENCES public.medication_reminders(id) ON DELETE SET NULL,
...
CREATE INDEX idx_alerts_elder ON public.alerts(elder_id);
CREATE INDEX idx_alerts_type ON public.alerts(alert_type);
CREATE INDEX idx_alerts_severity ON public.alerts(severity);
CREATE INDEX idx_alerts_status ON public.alerts(status);
CREATE INDEX idx_alerts_created_at ON public.alerts(created_at DESC);
CREATE INDEX idx_alerts_pending ON public.alerts(elder_id, status) WHERE status = 'pending';
...
CREATE POLICY "Family members can view alerts for their elders"
    ...
    USING (
        elder_id IN (
            SELECT elder_id
            FROM public.elder_family_relations
...
COMMENT ON TABLE public.alerts IS '警示系統表';
COMMENT ON COLUMN public.alerts.alert_type IS '警示類型：medication, health, activity, emergency';
COMMENT ON COLUMN public.alerts.severity IS '嚴重程度：low, medium, high, critical';

-- ----------------------------------------------------------------------------
-- 7.7.2 家屬設定表 (family_settings)
-- ----------------------------------------------------------------------------
CREATE TABLE public.family_settings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    family_member_id UUID NOT NULL REFERENCES public.family_members(id) ON DELETE CASCADE,
    notification_preferences JSONB DEFAULT '{"email": true, "sms": false, "push": true}'::jsonb,
    alert_thresholds JSONB DEFAULT '{"medication_compliance": 70, "missed_medication": 2, "safe_zone_alert": true}'::jsonb,
...
CREATE INDEX idx_family_settings_family_member_id ON public.family_settings(family_member_id);
...
CREATE POLICY "家屬可以刪除自己的設定"
    ON public.family_settings FOR DELETE
    USING (
        family_member_id IN (
            SELECT id FROM public.family_members WHERE auth_user_id = auth.uid()
        )
...
COMMENT ON TABLE public.family_settings IS '家屬個人化設定表';
COMMENT ON COLUMN public.family_settings.notification_preferences IS '通知偏好 (email, sms, push)';
COMMENT ON COLUMN public.family_settings.alert_thresholds IS '警示閾值設定';
...
CREATE TRIGGER update_chat_group_invites_updated_at
    BEFORE UPDATE ON chat_group_invites
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trigger_daily_reminder_logs_updated_at
    BEFORE UPDATE ON daily_reminder_logs
    FOR EACH ROW
    EXECUTE FUNCTION update_daily_reminder_updated_at();

-- 更新每日提醒統計
CREATE OR REPLACE FUNCTION update_daily_reminder_stats()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_daily_reminder_stats
    AFTER UPDATE ON daily_reminder_logs
    FOR EACH ROW
