...
-- ============================================================================
-- 版本: 2.1 (2025-01-27 更新)
-- 更新內容:
--   - 添加依賴檢查，如果基礎資料表不存在會給出清晰的錯誤訊息
...
BEGIN
    ...
    IF array_length(missing_tables, 1) > 0 THEN
        RAISE EXCEPTION E'❌ 錯誤：缺少必要的資料表！\n\n缺少的資料表: %\n\n請先執行主要的資料庫 Schema（supabase_complete_schema_with_auth_v4.sql）\n然後再執行此地理位置功能 Schema。', array_to_string(missing_tables, ', ');
    END IF;

    RAISE NOTICE '✅ 依賴檢查通過：所有必要的資料表都存在';
END $$;
...
RAISE NOTICE '✅ 已創建 safe_zones 資料表';
...
RAISE NOTICE '✅ 已創建 location_history 資料表';
...
RAISE NOTICE '✅ 已創建 geofence_alerts 資料表';
...
RAISE NOTICE '✅ 已創建 family_geolocation_settings 資料表';
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '📊 已創建資料表：';
    RAISE NOTICE '   • safe_zones (安全區域)';
    RAISE NOTICE '   • location_history (位置歷史)';
    RAISE NOTICE '   • geofence_alerts (地理圍欄警示)';
    RAISE NOTICE '   • family_geolocation_settings (家屬通知設定)';
    RAISE NOTICE '';
    RAISE NOTICE '🔒 已啟用 Row Level Security (RLS) - 11 個政策';
    RAISE NOTICE '⚙️ 已創建 4 個輔助函數';
    RAISE NOTICE '🔗 已創建 6 個索引';
    RAISE NOTICE '';
    RAISE NOTICE '🎉 下一步：請在應用程式中測試位置追蹤功能';
    RAISE NOTICE '   1. 重新整理家屬監控面板頁面';
    RAISE NOTICE '   2. 點擊「位置追蹤」標籤';
    RAISE NOTICE '   3. 點擊「📍 手動記錄位置」按鈕';
    RAISE NOTICE '';
END $$;
